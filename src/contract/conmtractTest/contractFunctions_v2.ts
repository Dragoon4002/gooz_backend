...
// NEW ABI - Players deposit for themselves (no _playerAddress parameter)
const ABI = [
  {
    "inputs": [
      {
...
// Owner contract instance (for prizeWithdrawal and admin functions)
...
/**
 * Deposit entry fee - PLAYER DEPOSITS FOR THEMSELVES
 * @param gameId - The game ID
 * @param playerPrivateKey - The player's private key (player signs their own transaction)
 */
export async function playerDeposit(gameId: string, playerPrivateKey: string): Promise<DepositResult> {
  try {
    // Create wallet for this specific player
    const playerWallet = new ethers.Wallet(playerPrivateKey, provider);
    const playerContract = new ethers.Contract(CONTRACT_ADDRESS, ABI, playerWallet);
    const playerAddress = playerWallet.address;

    const gameIdBytes32 = gameIdToBytes32(gameId);
    const entryFee = await playerContract.ENTRY_FEE();

    console.log(`💰 Player ${playerAddress} depositing...`);
    console.log(`   Game ID: ${gameId}`);
    console.log(`   Entry Fee: ${formatU2U(entryFee)} U2U`);

    // Check if already deposited
    const hasDeposited = await playerContract.hasPlayerDeposited(gameIdBytes32, playerAddress);
    if (hasDeposited) {
      throw new Error("Player already deposited for this game");
    }

    // Player signs and sends their own transaction
    const tx = await playerContract.playerDeposit(gameIdBytes32, {
      value: entryFee
    });
    ...
    const playerCount = await playerContract.getPlayerCount(gameIdBytes32);
...
export async function prizeWithdrawal(
  ...
  try {
    if (!ownerContract) {
      throw new Error("Owner contract not initialized. Check PRIVATE_KEY environment variable.");
    }
    ...
    if (receipt.logs && receipt.logs.length > 0) {
      for (const log of receipt.logs) {
        try {
          const parsedLog = ownerContract.interface.parseLog({
            topics: [...log.topics],
            data: log.data
          });
...
/**
 * Emergency withdrawal (owner only) - refunds all players
 */
export async function emergencyWithdraw(gameId: string): Promise<EmergencyWithdrawResult> {
  try {
    ...
    const details = await getGameDetails(gameId);
    console.log(`   Refunding ${details.players.length} players (${formatU2U(details.poolAmount)} U2U total)`);
...
export async function getGameDetails(gameId: string): Promise<GameDetails> {
  try {
    const contract = ownerContract || new ethers.Contract(CONTRACT_ADDRESS, ABI, provider);
    const gameIdBytes32 = gameIdToBytes32(gameId);
...
export async function setCreatorWallet(newCreatorWallet: string): Promise<{ success: boolean; transactionHash: string; blockNumber: number }> {
  try {
    if (!ownerContract) {
      throw new Error("Owner contract not initialized.");
    }

    console.log(`🔄 Updating creator wallet to ${newCreatorWallet}...`);

    const tx = await ownerContract.setCreatorWallet(newCreatorWallet);
